...
export const BeveragesMenu = () => {
  const [beverages, setBeverages] = useState<FoodsType[]>([]);
  const getFoods = async () => {
    const response = await axios.get(
      "http://localhost:3001/food?categoryId=6807438c58747cab008b88f6"
    );
    setBeverages(response.data.food);
  };
  useEffect(() => {
    getFoods();
  }, []);
  ...
  return (
    <div className="bg-white p-5 rounded-xl flex flex-col gap-4">
      <h4 className="text-xl font-semibold">
        {beverages[0]?.category?.categoryName}
      </h4>
